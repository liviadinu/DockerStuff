#example New-MvDevContainer -containerName mvx-dev -dbcontainername mvxdb -licenseFile "C:\temp\K3-5194146-22022018.flf" -uidOffset 74000 -createNewDb
#container params
...
new-navcontainer -accept_eula -containername $hostname -imageName $navImageNameTag -auth NavUserPassword -includecside -updateHosts -enableSymbolLoading `
                 -licenseFile $licenseFile -doNotExportObjectsToText -Credential $dbcred -accept_outdated `
                 -databaseServer $dbcontainername -databaseName $dbname -databaseCredential $dbcred `
                 -AdditionalParameters @("-e locale=en-NL -p 20-21:20-21 --restart unless-stopped") 
