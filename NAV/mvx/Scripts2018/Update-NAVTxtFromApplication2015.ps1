...
Begin {
    ...
    Import-Module -Name NVR_NAVScripts -Force -DisableNameChecking
    Import-NAVModelTool
...
Process {
    ...
    foreach ($NAVObject in $NAVObjects)
    {
        ...
        else 
        {
            ...
            else 
            {
                ...
                {
                    Write-Host -Object "$Type $Id differs: Modified=$($FileObject.Modified -eq $NAVObject.Modified) Version=$($FileObject.VersionList -eq $NAVObject.'Version List') Time=$($FileObject.Time.TrimStart(' ') -eq $NAVObject.Time.ToString('HH:mm:ss')) Date=$($FileObject.Date -eq $NAVObject.Date.ToString('dd-MM-yy'))"
                }
            }
        }
    }
    ...
    if ($AllInOne) 
    {
        ...
        Export-NAVApplicationObject -Filter 'Compiled=0|1' -DatabaseServer $Server -DatabaseName $Database -LogPath 'c:\temp\LOG\' -path (Join-Path -Path $Path -ChildPath 'all.txt')
    ...
    else 
    {
        foreach ($updateobject in $UpdatedObjects) 
        {
            ...
            Export-NAVApplicationObject -Filter $updateobject.Filter -DatabaseServer $Server -DatabaseName $Database -LogPath 'c:\temp\LOG\' -path $updateobject.TargetFileName -Force
        }
...
End {
    Remove-Item -Path 'LOG' -Force -ErrorAction Ignore
}